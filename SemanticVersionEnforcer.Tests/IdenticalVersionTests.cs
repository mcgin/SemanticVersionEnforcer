...
{
    internal class IdenticalVersionTests : SemanticVersionBase
    {
        ...
        public void GivenTwoPackages_WhenTheyBothContainTheSameAbstractClass_ItShouldHaveTheySameVersionNumbers()
        {
            ...
            Assert.AreEqual(new Version(2, 3),
        ...
        public void GivenTwoPackages_WhenTheNewerOneContainsAdditionalPrivateMethods_ItShouldHaveTheSameVersion()
        {
            ...
            const String newSource =
                "public class B { public void hello() { int x=7; } private void hello2() { int x=7; } }";
        ...
        public void GivenTwoIdenticalPackages_WhenTheSemanticVersionIsCalculated_ItShouldBeTheSameVersionAsTheOldOne()
        {
            ...
            var mockDll = new Mock<IPackageFile>();
            mockDll.Setup(dll => dll.EffectivePath).Returns("test.dll");
            mockDll.Setup(dll => dll.GetStream())
                .Returns(File.Open("TestData/nunit.framework.2.6.2.dll", FileMode.Open));
            ...
            var newMockDll = new Mock<IPackageFile>();
            newMockDll.Setup(dll => dll.EffectivePath).Returns("test.dll");
            newMockDll.Setup(dll => dll.GetStream())
                .Returns(File.Open("TestData/nunit.framework.copy.2.6.2.dll", FileMode.Open));
                //For some reason I can't get it to work using the same mock

            var newMockPackage = new Mock<IPackage>();
            newMockPackage.Setup(p => p.GetFiles()).Returns(new List<IPackageFile> {newMockDll.Object});

            var checker = new SemanticVersionChecker();
            Assert.AreEqual(new Version(2, 6),
                checker.DetermineCorrectSemanticVersion(mockPackage.Object, newMockPackage.Object));
        }

        [Test]
        public void
            GivenTwoIdenticalPackages_WhenTheSemanticVersionIsCalculatedByPassingTheFilesAsStrings_ItShouldBeTheSameVersionAsTheOldOne
            ()
        {
            var checker = new SemanticVersionChecker();
            Assert.AreEqual(new Version(1, 0),
                checker.DetermineCorrectSemanticVersion("TestData/SemanticVersionEnforcer.1.0.1.0.nupkg",
                    "TestData/SemanticVersionEnforcer.1.0.1.0.nupkg"));
        }
